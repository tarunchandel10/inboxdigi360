...
import { getAllAdvertiserUser, getAllPanelUsersAdvertiser } from '../api/advertiser';
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (userProfile.user_type === ADVERTISER) {
          const data = await getAllPanelUsersAdvertiser(currentPage);
          if (data.status) {
            setUsers(data.data);
...
export default withAuthAndRole(PanelUsers, [ADVERTISER]);
