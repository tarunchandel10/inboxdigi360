...
export default function CarrierISPPopup({ onApply,isEdit,dataSelected,isAdded }) {
    ...
    useEffect(() => {
        ...
        if (dataSelected && dataSelected.length > 0 && languages.length > 0) {
    ...
    const fetchLanguageData = async () => {
        ...
        try {
            ...
            const response = await fetch(`${Baseurl}all-carrier-isp-list`, {
                headers: {
    ...
    const searchLanguage = async () => {
        try {
            ...
            const response = await fetch(`${Baseurl}search-carrier-isp-list?keyword=${searchKeyword}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
...
{isAdded ?<button className="addedButton">Added</button>:
            ...
            >
                <Modal.Header className={`${FormCss.modal_header}`}>
                    <Modal.Title className={`${FormCss.modal_title}`}>Carrier & ISP</Modal.Title>
